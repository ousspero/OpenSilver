...
md temp
@echo OpenSilver.Simulator SL.WorkInProgress 1.0.0-alpha-%PackageVersion% (%currentdate% %currenttime%)> temp/Version.txt
...
echo. 
msbuild slnf/OpenSilver.Simulator.slnf -p:Configuration=SL.WorkInProgress -restore
echo. 
echo %ESC%[95mPacking %ESC%[0mOpenSilver.WorkInProgress.Simulator %ESC%[95mNuGet package%ESC%[0m
echo. 
nuget.exe pack nuspec\OpenSilver.Simulator.nuspec -OutputDirectory "output/OpenSilver" -Properties "PackageId=OpenSilver.WorkInProgress.Simulator;PackageVersion=1.0.0-alpha-%PackageVersion%;Configuration=SL.WorkInProgress"
